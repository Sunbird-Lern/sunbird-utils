...
public class RecordProcessor extends StatusTracker {
    ...
    private Map<String,String> getOrgDataFromDbAsList() {
        ResultSet resultSet = connection.getRecords("select * from organisation where isrootorg=true allow filtering");
    ...
    public void startProcessingExternalIds() throws IOException {
        ...
        int count[] = new int[1];
        logger.info("==================Starting Processing Self Declared Users=======================================");
        ...
        logger.info("==================Endingrting Processing Self Declared Users=======================================");
        ...
        List<User> stateUsersToBeProcessed =  removePreProcessedRecordFromList(preProcessedRecords, stateUsersList);
        logger.info("==================Starting Processing State Users=======================================");
        //Update provider to OrgId
        int countUpdateUser []= new int[1];
        stateUsersToBeProcessed.stream().forEach(
                stateUser->{
                     try{
                         startTracingRecord(stateUser.getUserId());
                         boolean isSuccess = performSequentialUpdateOperationOnRecord(stateUser,orgIdProviderMap);
                         if(isSuccess){
                             countUpdateUser[0] +=1;
                         }
                     }catch (Exception ex){
                         logExceptionOnProcessingRecord(stateUser.getUserId(),stateUser.getOriginalProvider(),
        ...
        logger.info("==================Ending Processing State Users=======================================");

        connection.closeConnection();
        closeFw2WriterConnection();
        logger.info("Total Records: "+ userSelfDeclareLists.size()+ " Successfully Self Declared Users Migrated: "+count[0]+ " Total Self Declared Users Migration Failed:" + (userSelfDeclareLists.size()-count[0]));
        logger.info("Total state users after migration:"+stateUsersList.size());
        logger.info("Total State Users to be Updated: "+ stateUsersList.size()+ "  SuccessFully State Users Updated: "+countUpdateUser[0]+ " Total State User Update Failed: "+ (stateUsersList.size()- countUpdateUser[0]));


    }

    private boolean performSequentialUpdateOperationOnRecord(User stateUser, Map<String, String> orgIdProviderMap) {
    ...
    private boolean performSequentialOperationOnRecord(
        ...
        try {
            ...
            if(isUpdateOperation) {
                logSelfDeclaredSuccessRecord(userDeclareEntity.getUserId(),userDeclareEntity.getProvider());
                return true;
            }else {

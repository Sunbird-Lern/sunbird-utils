...
public class TelemetryDispatcherEkstep implements TelemetryDispatcher {
  ...
  public boolean dispatchTelemetryEvent(List<String> eventList) {
    ...
    try {
      ...
      long count =
          jsonList.stream().filter(event -> "AUDIT".equals((String) event.get("eid"))).count();
      ProjectLogger.log("Audit events count: " + count, LoggerEnum.INFO.name());
      String event = getTelemetryEvent(map);

      String response =
          HttpUtil.sendPostRequest(
              getCompleteUrl(JsonKey.EKSTEP_BASE_URL, JsonKey.EKSTEP_TELEMETRY_API_URL),
              event,
              getEkstepHeaders());
      ProjectLogger.log(
          "Ekstep Telemetry flush response. Audit count: " + count,
          response,

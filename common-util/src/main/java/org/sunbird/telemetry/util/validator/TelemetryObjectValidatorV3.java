...
public class TelemetryObjectValidatorV3 implements TelemetryObjectValidator {
	...
	public boolean validateAudit(String jsonString) {
		...
		try {
			...
			if (!missingFields.isEmpty()) {
				ProjectLogger.log("Telemetry Object Creation Error for event : " + TelemetryEvents.AUDIT.getName()
	...
	public boolean validateSearch(String jsonString) {
		...
		try {
			...
			if (!missingFields.isEmpty()) {
				ProjectLogger.log("Telemetry Object Creation Error for event : " + TelemetryEvents.SEARCH.getName()
						+ "  missing required fields :" + String.join(",", missingFields));
				validationSuccess = false;
			}
		} catch (IOException e) {
	...
	public boolean validateLog(String jsonString) {
		...
		try {
			...
			if (!missingFields.isEmpty()) {
				ProjectLogger.log("Telemetry Object Creation Error for event : " + TelemetryEvents.LOG.getName()
						+ "  missing required fields :" + String.join(",", missingFields));
				validationSuccess = false;
			}
		} catch (IOException e) {
	...
	public boolean validateError(String jsonString) {
		...
		try {
			...
			if (!missingFields.isEmpty()) {
				ProjectLogger.log("Telemetry Object Creation Error for event : " + TelemetryEvents.ERROR.getName()
						+ "  missing required fields :" + String.join(",", missingFields));
				validationSuccess = false;
			}
		} catch (IOException e) {

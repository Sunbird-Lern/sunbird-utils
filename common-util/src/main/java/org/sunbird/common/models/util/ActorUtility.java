		...
		try {
			...
			try {
				if (!ProjectUtil.isStringNullOREmpty(System.getenv(JsonKey.SUNBIRD_ACTOR_IP))
					...
					path = MessageFormat.format(PropertiesCache.getInstance().getProperty("remote.actor.env.path"),
			...
			ProjectLogger
					.log("Exception Occurred while creating remote connection in ActorUtility createConnection method "
							+ ex);
		}
	}

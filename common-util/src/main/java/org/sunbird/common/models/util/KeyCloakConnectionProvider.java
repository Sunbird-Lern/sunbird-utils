...
/**
 * @author Manzarul This class will connect to key cloak server and provide the
 *         connection to do other operations.
 */
...
public class KeyCloakConnectionProvider {
	...
	public static Keycloak initialiseConnection() throws Exception {
		...
		KeycloakBuilder keycloakBuilder = KeycloakBuilder.builder().serverUrl(cache.getProperty(JsonKey.SSO_URL))
				.realm(cache.getProperty(JsonKey.SSO_REALM)).username(cache.getProperty(JsonKey.SSO_USERNAME))
				.password(cache.getProperty(JsonKey.SSO_PASSWORD)).clientId(cache.getProperty(JsonKey.SSO_CLIENT_ID))
	...
	/**
	 * This method will provide the keycloak connection from environment variable.
	 * if environment variable is not set then it will return null.
	 * 
	 * @return Keycloak
	 */
	private static Keycloak initialiseEnvConnection() throws Exception {
		...
		SSO_REALM = relam;
		CLIENT_ID = cleintId;
		KeycloakBuilder keycloakBuilder = KeycloakBuilder.builder().serverUrl(url).realm(relam).username(username)
		...
		keycloak = keycloakBuilder.build();
		ProjectLogger.log("key cloak instance is created from Environment variable settings .", LoggerEnum.INFO.name());
		return keycloak;

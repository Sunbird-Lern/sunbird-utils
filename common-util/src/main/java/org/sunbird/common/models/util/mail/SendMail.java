...
public class SendMail {
  ...
  static {
    ...
    if (ProjectUtil.isStringNullOREmpty(host)
      ...
      ProjectLogger.log("Email setting value is not provided by Env variable=="
          + host + " " + port + " " + fromEmail, LoggerEnum.INFO.name());
      host =
    ...
    try {
      ...
      while (size > 0) {
        message.addRecipient(Message.RecipientType.TO,
            new InternetAddress(receipent[i]));
        i++;
        size--;
      }
    ...
    public static void sendMail(String[] receipent, String subject,
	    VelocityContext context, String templateName, String[] ccList) {
      Transport transport = null;
	try {
	    ...
	    VelocityEngine engine = new VelocityEngine();
	    engine.init();
	    String templatePath = PropertiesCache.getInstance()
		    .getProperty("email.template.basepath");
     ...
     *            String
     */
    public static void sendAttachment(String[] receipent, String mail,

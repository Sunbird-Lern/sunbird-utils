...
public class ConfigUtil {
  ...
  public static Config getConfigFromJsonString(String jsonString) {
    ProjectLogger.log(
        "ConfigUtil:getConfigFromJsonString: Parsing json string to config object",
        LoggerEnum.INFO.name());
    if (null == jsonString || StringUtils.isBlank(jsonString)) {
      ProjectLogger.log(
          "ConfigUtil:getConfigFromJsonString: Empty string is passed for parsing",
          LoggerEnum.ERROR.name());
    ...
    try {
      ...
      ProjectLogger.log(
          "ConfigUtil:getConfigFromJsonString: Error while parsing json string to config object",
          LoggerEnum.ERROR.name());
    ...
    if (null == jsonConfig || jsonConfig.isEmpty()) {
      ProjectLogger.log(
          "ConfigUtil:getConfigFromJsonString: Config object is empty", LoggerEnum.ERROR.name());
      ProjectCommonException.throwServerErrorException(ResponseCode.errorConfigLoadEmptyConfig, "");
    }
    ProjectLogger.log(
        "ConfigUtil:getConfigFromJsonString: Json string is successfully parsed into a config object",
        LoggerEnum.INFO.name());
    return jsonConfig;
  }
}

...
public class ConfigUtil {
   ...
   * @return Type safe config object
   */
  public static Config getConfigFromJsonString(String jsonString) {
    ProjectLogger.log(
        "ConfigUtil: getConfigFromJsonString called",
        LoggerEnum.DEBUG.name());

    if (null == jsonString || StringUtils.isBlank(jsonString)) {
      ProjectLogger.log(
          "ConfigUtil:getConfigFromJsonString: Empty string",
          LoggerEnum.ERROR.name());
    ...
    try {
      ...
      ProjectLogger.log(
          "ConfigUtil:getConfigFromJsonString: Exception occurred during parse with error message = " + e.getMessage(),
          LoggerEnum.ERROR.name());

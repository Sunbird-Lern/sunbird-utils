...
public class DefaultAssessmentEvaluator implements AssessmentEvaluator {
	 ...
	 * 
	 * @param evaluatedData
	 *            Map<String, List<Map<String, Object>>> (map of userId, list of
	 *            assessment obj)
	...
	public List<Map<String, Object>> evaluateResult(Map<String, List<Map<String, Object>>> evaluatedData) {
		...
		String tempKey = (String) request.get(0).get(JsonKey.COURSE_ID) + "#"
		...
		if (assmntMap != null && assmntMap.size() > 0) {
			...
			while (itr.hasNext()) {
				...
				for (Map<String, Object> map : list) {
					...
					score = score + (((String) map.get(JsonKey.ASSESSMENT_SCORE)) != null
	...
	private List<Map<String, Object>> getAssessmentListWithMaxScore(Entry<String, List<Map<String, Object>>> entry) {
		...
		if (request.size() > 1) {
			for (int i = 1; i < request.size(); i++) {
				...
				score = (request.get(i).get(JsonKey.ASSESSMENT_SCORE)) != null
				...
				if (assessmentWithMaxScore.containsKey(tempKey)) {
					...
					if (null != tempMap.get(JsonKey.ASSESSMENT_SCORE) && Double.compare(score,
							Double.parseDouble((String) tempMap.get(JsonKey.ASSESSMENT_SCORE))) > 0) {

...
public final class RequestValidator {
    ...
    /**
     * This method will do course enrollment request data validation. if all mandatory data is
    ...
    /**
     * This method will do content state request data validation. if all mandatory data is coming
    ...
    public static void validateUpdateContent(Request contentRequestDto) {
        if (((List<Map<String, Object>>) (contentRequestDto.getRequest().get(JsonKey.CONTENTS)))
            ...
            List<Map<String, Object>> list = (List<Map<String, Object>>) (contentRequestDto
    ...
    public static void validateCreateOrg(Request request) {
        ...
        if ((null != request.getRequest().get(JsonKey.IS_ROOT_ORG)
                ...
                && ProjectUtil
                        .isStringNullOREmpty((String) request.getRequest().get(JsonKey.CHANNEL))) {
            throw new ProjectCommonException(
    ...
    public static void validateGetPageData(Request request) {
        if (request == null
                || (StringUtils.isBlank((String) request.get(JsonKey.SOURCE)))) {
    ...
    public static void validateCreateSection(Request request) {
        if (StringUtils.isBlank(
                (String) (request.getRequest().get(JsonKey.SECTION_NAME) != null
        ...
        if (StringUtils.isBlank(
                (String) (request.getRequest().get(JsonKey.SECTION_DATA_TYPE) != null
    ...
    public static void validateUpdateSection(Request request) {
        ...
        if (StringUtils.isBlank((String) (request.getRequest().get(JsonKey.ID) != null
    ...
    public static void validateUpdatepage(Request request) {
        if (request.getRequest().containsKey(JsonKey.PAGE_NAME) && ProjectUtil
                .isStringNullOREmpty((String) (request.getRequest().get(JsonKey.PAGE_NAME) != null
                        ? request.getRequest().get(JsonKey.PAGE_NAME) : ""))) {
            throw new ProjectCommonException(ResponseCode.pageNameRequired.getErrorCode(),
                    ResponseCode.pageNameRequired.getErrorMessage(), ERROR_CODE);
    ...
    private static void validateAssessment(List<Map<String, Object>> list) {
        for (Map<String, Object> map : list) {
            ...
            if (StringUtils.isBlank((String) (map.get(JsonKey.ASSESSMENT_TYPE) != null
    ...
    public static void validateUploadUser(Request reqObj) {
        if (ProjectUtil
                ...
                && (ProjectUtil
                        .isStringNullOREmpty((String) reqObj.getRequest().get(JsonKey.EXTERNAL_ID))
                        || StringUtils.isBlank(
     ...
     * "invite-only" enrolmentType. for open type if createdFor values is coming then system will
     * just save that value. mentors : List of user ids , who will work as a mentor.
     * 
     * @param request
     */
    ...
    public static void validateCreateBatchReq(Request request) {
        ...
        if (request.getRequest().containsKey(JsonKey.END_DATE) && !ProjectUtil
                .isStringNullOREmpty((String) request.getRequest().get(JsonKey.END_DATE))) {
    ...
    public static void validateUpdateCourseBatchReq(Request request) {
        ...
        if (request.getRequest().containsKey(JsonKey.END_DATE) && !ProjectUtil
                .isStringNullOREmpty((String) request.getRequest().get(JsonKey.END_DATE))) {
            boolean bool =
                    validateDateWithTodayDate((String) request.getRequest().get(JsonKey.END_DATE));
            if (!bool) {
                throw new ProjectCommonException(
    ...
    private static boolean validateBatchStatus(Request request) {
        ...
        try {
            status = checkProgressStatus(
                    Integer.parseInt("" + request.getRequest().get(JsonKey.STATUS)));
    ...
    private static void validateUpdateBatchEndDate(Request request) {
        ...
        if (request.getRequest().containsKey(JsonKey.END_DATE)
                ...
                && !StringUtils.isBlank(
                        (String) request.getRequest().get(JsonKey.START_DATE))) {
            Date batchStartDate = null;
            Date batchEndDate = null;
            try {
                batchStartDate =
                        format.parse((String) request.getRequest().get(JsonKey.START_DATE));
    ...
    public static void validateSyncRequest(Request request) {
        ...
        if ((null != operation) && (!operation.equalsIgnoreCase("keycloak"))) {
            ...
            List<String> list = new ArrayList<>(Arrays.asList(new String[] {JsonKey.USER,
                    JsonKey.ORGANISATION, JsonKey.BATCH, JsonKey.USER_COURSE}));
    ...
    public static void validateUpdateSystemSettingsRequest(Request request) {
        List<String> list = new ArrayList<>(Arrays.asList(PropertiesCache.getInstance()
                .getProperty("system_settings_properties").split(",")));
        for (String str : request.getRequest().keySet()) {
            if (!list.contains(str)) {
                throw new ProjectCommonException(
                        ResponseCode.invalidPropertyError.getErrorCode(), MessageFormat
                                .format(ResponseCode.invalidPropertyError.getErrorMessage(), str),
                        ERROR_CODE);
            }
        }
    }
    ...
    public static void validateSendMail(Request request) {
        ...
        if ((null != (request.getRequest().get(JsonKey.RECIPIENT_EMAILS))
                ...
                && (null != (request.getRequest().get(JsonKey.RECIPIENT_USERIDS))
                        && ((List) request.getRequest().get(JsonKey.RECIPIENT_USERIDS))
                                .isEmpty())) {
            throw new ProjectCommonException(ResponseCode.recipientAddressError.getErrorCode(),
                    ResponseCode.recipientAddressError.getErrorMessage(), ERROR_CODE);
        }
    ...
    public static void validateGetUserCount(Request request) {
        if (!validateListType(request, JsonKey.LOCATION_IDS)) {
            throw createDataTypeException(ResponseCode.dataTypeError.getErrorCode(),
    ...
    private static ProjectCommonException createDataTypeException(String errorCode, String key1,
            String key2) {
        return new ProjectCommonException(ResponseCode.getResponse(errorCode).getErrorCode(),
                ProjectUtil.formatMessage(ResponseCode.getResponse(errorCode).getErrorMessage(),
                        key1, key2),
                ERROR_CODE);
    }

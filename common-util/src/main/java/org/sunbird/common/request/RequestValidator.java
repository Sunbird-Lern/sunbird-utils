...
public final class RequestValidator {
	...
	/**
	 * This method will do course enrollment request data validation. if all
	 * mandatory data is coming then it won't do any thing if any mandatory data is
	 * missing then it will throw exception.
	 * 
	...
	public static void validateUpdateOrg(Request request) {
		...
		if (request.getRequest().containsKey(JsonKey.ROOT_ORG_ID)
				&& ProjectUtil.isStringNullOREmpty((String) request.getRequest().get(JsonKey.ROOT_ORG_ID))) {
	...
	public static void validateGetPageData(Request request) {
		if (request == null || (ProjectUtil.isStringNullOREmpty((String) request.get(JsonKey.SOURCE)))) {
	...
	public static void validateUpdateSection(Request request) {
		...
		if (ProjectUtil.isStringNullOREmpty(
				(String) (request.getRequest().get(JsonKey.ID) != null ? request.getRequest().get(JsonKey.ID) : ""))) {
	...
	private static void validateAssessment(List<Map<String, Object>> list) {
		for (Map<String, Object> map : list) {
			if (ProjectUtil.isStringNullOREmpty(
					(String) (map.get(JsonKey.ASSESSMENT_ITEM_ID) != null ? map.get(JsonKey.ASSESSMENT_ITEM_ID)
							: ""))) {
			...
			if (ProjectUtil.isStringNullOREmpty(
					(String) (map.get(JsonKey.ASSESSMENT_TYPE) != null ? map.get(JsonKey.ASSESSMENT_TYPE) : ""))) {
			...
			if (ProjectUtil.isStringNullOREmpty(
					(String) (map.get(JsonKey.ASSESSMENT_ANSWERS) != null ? map.get(JsonKey.ASSESSMENT_ANSWERS)
							: ""))) {
			...
			if (ProjectUtil.isStringNullOREmpty(
					(String) (map.get(JsonKey.ASSESSMENT_MAX_SCORE) != null ? map.get(JsonKey.ASSESSMENT_MAX_SCORE)
							: ""))) {
	...
	private static void validateUpdateBatchEndDate(Request request) {
		...
		if (request.getRequest().containsKey(JsonKey.END_DATE)
				...
				&& !ProjectUtil.isStringNullOREmpty((String) request.getRequest().get(JsonKey.START_DATE))) {
	...
	public static void validateSyncRequest(Request request) {
		...
		if ((null != operation) && (!operation.equalsIgnoreCase("keycloak"))) {
			...
			List<String> list = new ArrayList<>(Arrays
					.asList(new String[] { JsonKey.USER, JsonKey.ORGANISATION, JsonKey.BATCH, JsonKey.USER_COURSE }));

...
public class UserRequestValidator {
	...
	public static void createUserBasicValidation(Request userRequest) {
		...
		if (userRequest.getRequest().containsKey(JsonKey.ROLES) && null != userRequest.getRequest().get(JsonKey.ROLES)
			...
			throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
					ResponseCode.dataTypeError.getErrorMessage(), JsonKey.ROLES, JsonKey.LIST), ERROR_CODE);
		}
		if (userRequest.getRequest().containsKey(JsonKey.LANGUAGE)
			...
			throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
					ResponseCode.dataTypeError.getErrorMessage(), JsonKey.LANGUAGE, JsonKey.LIST), ERROR_CODE);
		}
	...
	private static void addressValidation(Request userRequest) {
		...
		if (userRequest.getRequest().containsKey(JsonKey.ADDRESS)
			...
			if (!(userRequest.getRequest().get(JsonKey.ADDRESS) instanceof List)) {
				throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
						ResponseCode.dataTypeError.getErrorMessage(), JsonKey.ADDRESS, JsonKey.LIST), ERROR_CODE);
	...
	private static void educationValidation(Request userRequest) {
		...
		if (userRequest.getRequest().containsKey(JsonKey.EDUCATION)
			...
			if (!(userRequest.getRequest().get(JsonKey.EDUCATION) instanceof List)) {
				throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
						ResponseCode.dataTypeError.getErrorMessage(), JsonKey.EDUCATION, JsonKey.LIST), ERROR_CODE);
	...
	private static void jobProfileValidation(Request userRequest) {
		if (userRequest.getRequest().containsKey(JsonKey.JOB_PROFILE)
			...
			if (!(userRequest.getRequest().get(JsonKey.JOB_PROFILE) instanceof List)) {
				throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
						ResponseCode.dataTypeError.getErrorMessage(), JsonKey.JOB_PROFILE, JsonKey.LIST), ERROR_CODE);
	...
	private static void validateUpdateUserEducation(Request userRequest) {
		...
		for (int i = 0; i < reqList.size(); i++) {
			...
			if (!reqMap.containsKey(JsonKey.IS_DELETED) || (reqMap.containsKey(JsonKey.IS_DELETED)
					&& (null == reqMap.get(JsonKey.IS_DELETED) || !(boolean) reqMap.get(JsonKey.IS_DELETED)))) {
				educationValidation(userRequest);
			}
		}
	}
	...
	public static void validateProfileVisibility(Request request) {
		if (request.getRequest().get(JsonKey.PRIVATE) == null && request.getRequest().get(JsonKey.PUBLIC) == null) {
		...
		if (request.getRequest().containsKey(JsonKey.PRIVATE)
			...
			throw new ProjectCommonException(ResponseCode.dataTypeError.getErrorCode(), ProjectUtil.formatMessage(
					ResponseCode.dataTypeError.getErrorMessage(), JsonKey.PRIVATE, JsonKey.LIST), ERROR_CODE);
		}
		...
		if (null != request.getRequest().get(JsonKey.PRIVATE) && null != request.getRequest().get(JsonKey.PUBLIC)) {

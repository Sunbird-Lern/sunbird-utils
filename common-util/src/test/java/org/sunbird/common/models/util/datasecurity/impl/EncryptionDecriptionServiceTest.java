...
public class EncryptionDecriptionServiceTest {
	...
	public void testDataDecryptionFrMap() {
		try {
			assertEquals(decryptionService.decryptData(encryptionService.encryptData(map2)).get(JsonKey.FIRST_NAME),
					"Amit");
		} catch (Exception e) {
		}
	}
	...
	public void testDataDecryptionFrMapWithMapList() {
		try {
			map2.put(JsonKey.LOCATION, "");
			assertEquals(
					decryptionService.decryptData(encryptionService.encryptData(mapList2)).get(0).get(JsonKey.LOCATION),
					"");
		} catch (Exception e) {
		}
	}
	...
	public void testBDataDecryption() {
		try {
			if (JsonKey.ON.equalsIgnoreCase(sunbirdEncryption)) {
				assertNotEquals(encryptionService.encryptData("Hello"),

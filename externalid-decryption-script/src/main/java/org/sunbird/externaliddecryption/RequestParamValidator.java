...
import org.sunbird.externaliddecryption.constants.EnvConstants;

public class RequestParamValidator {

  private static Logger logger =
      LoggerFactory.getLoggerInstance(ExternalIdDecryption.class.getName());
  ...
  private static boolean isEnvValidated(RequestParams requestParams) {
    ...
    if (isInvalidEnv(requestParams.getSunbirdEncryptionKey())) {
      throw new IllegalArgumentException(
          String.format("Valid %s is required", EnvConstants.SUNBIRD_ENCRYPTION_KEY));
    }

...
/**
 * This class will create elastic search query from requested search data.
 * 
 * @author Manzarul
 *
 */
public class ElasticSearchQueryBuilder {
  ...
  public static void createIndex(String indexName, String typeName) {
    ...
    try {
      boolean response = client.admin().indices().exists(Requests.indicesExistsRequest(indexName))
          .get().isExists();
      TypesExistsRequest typesExistsRequest =
          new TypesExistsRequest(new String[] {indexName}, typeName);
      boolean typeResponse =
          client.admin().indices().typesExists(typesExistsRequest).get().isExists();
    ...
    PutMappingResponse response = client.admin().indices().preparePutMapping(indexName)
        .setType(typeName).setSource(createMapping()).get();
  }

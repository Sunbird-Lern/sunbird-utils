...
public class ElasticSearchUtil {
    ...
    public static String createData(String index, String type, String identifier,
        ...
        try {
            ...
            IndexResponse response = ConnectionManager.getClient()
                    .prepareIndex(index, type, identifier).setSource(data).get();
            ProjectLogger.log("Save value==" + response.getId() + " " + response.status(),
                    LoggerEnum.INFO.name());
            ProjectLogger.log("ElasticSearchUtil createData method end at =="
                    + System.currentTimeMillis() + " for Type " + type + " ,Total time elapsed = "
                    + calculateEndTime(startTime), LoggerEnum.PERF_LOG);
    ...
    /**
     * This method will provide data form ES based on incoming identifier. we can get data by
     * passing index and identifier values , or all the three
     *
     * @param type String
    ...
    public static Map<String, Object> getDataByIdentifier(String index, String type,
        ...
        ProjectLogger.log("ElasticSearchUtil getDataByIdentifier method end at ==" + stopTime
                + " for Type " + type + " ,Total time elapsed = " + elapsedTime,
                LoggerEnum.PERF_LOG);
    ...
    public static Map<String, Object> searchData(String index, String type,
        ...
        long startTime = System.currentTimeMillis();
        ProjectLogger.log("ElasticSearchUtil searchData method started at ==" + startTime
                + " for Type " + type, LoggerEnum.PERF_LOG);
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil searchData method end at ==" + stopTime + " for Type "
                + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
    ...
    public static boolean updateData(String index, String type, String identifier,
        ...
        if (!ProjectUtil.isStringNullOREmpty(index) && !ProjectUtil.isStringNullOREmpty(type)
            ...
            try {
                ...
                if (response.getResult().name().equals("UPDATED")) {
                    ...
                    long elapsedTime = stopTime - startTime;
                    ProjectLogger.log(
                            "ElasticSearchUtil updateData method end at ==" + stopTime
                                    + " for Type " + type + " ,Total time elapsed = " + elapsedTime,
                            LoggerEnum.PERF_LOG);
                    return true;
                }
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil updateData method end at ==" + stopTime + " for Type "
                + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
        return false;
    }
    ...
    public static boolean upsertData(String index, String type, String identifier,
        ...
        if (!ProjectUtil.isStringNullOREmpty(index) && !ProjectUtil.isStringNullOREmpty(type)
            ...
            if (upsertResults.contains(response.getResult().name())) {
                ...
                long elapsedTime = stopTime - startTime;
                ProjectLogger.log(
                        "ElasticSearchUtil upsertData method end at ==" + stopTime + " for Type "
                                + type + " ,Total time elapsed = " + elapsedTime,
                        LoggerEnum.PERF_LOG);
                return true;
            }
        } else {
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil upsertData method end at ==" + stopTime + " for Type "
                + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
        return false;
    }
    ...
    public static boolean removeData(String index, String type, String identifier) {
        ...
        if (!ProjectUtil.isStringNullOREmpty(index) && !ProjectUtil.isStringNullOREmpty(type)
            ...
            try {
                ...
                ProjectLogger.log("delete info ==" + deleteResponse.getResult().name() + " "
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil removeData method end at ==" + stopTime
    ...
    public static boolean addOrUpdateMapping(String indexName, String typeName, String mapping) {
        try {
            PutMappingResponse response = ConnectionManager.getClient().admin().indices()
                    .preparePutMapping(indexName).setType(typeName).setSource(mapping).get();
    ...
    /**
     * Method to perform the elastic search on the basis of SearchDTO . SearchDTO contains the
     * search criteria like fields, facets, sort by , filters etc. here user can pass single type to
     * search or multiple type or null
     *
    ...
    public static Map<String, Object> complexSearch(SearchDTO searchDTO, String index,
            String... type) {
        long startTime = System.currentTimeMillis();
        ProjectLogger.log("ElasticSearchUtil complexSearch method started at ==" + startTime,
                LoggerEnum.PERF_LOG);
        ...
        if (searchDTO.getSortBy() != null && searchDTO.getSortBy().size() > 0) {
            for (Map.Entry<String, String> entry : searchDTO.getSortBy().entrySet()) {
                searchRequestBuilder.addSort(entry.getKey() + RAW_APPEND,
                        getSortOrder(entry.getValue()));
            }
        }
        ...
        searchRequestBuilder.setFetchSource(
                searchDTO.getFields() != null
                        ? searchDTO.getFields().stream().toArray(String[]::new) : null,
                searchDTO.getExcludedFields() != null
                        ? searchDTO.getExcludedFields().stream().toArray(String[]::new) : null);
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil complexSearch method end at ==" + stopTime
                + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
        return responsemap;
    }

    private static void addAggregations(SearchRequestBuilder searchRequestBuilder,
            List<Map<String, String>> facets) {
        long startTime = System.currentTimeMillis();
        ProjectLogger.log("ElasticSearchUtil addAggregations method started at ==" + startTime,
                LoggerEnum.PERF_LOG);
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            if (JsonKey.DATE_HISTOGRAM.equalsIgnoreCase(value)) {
                searchRequestBuilder.addAggregation(
                        AggregationBuilders.dateHistogram(key).field(key + RAW_APPEND)
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil addAggregations method end at ==" + stopTime
                + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
    }
    ...
    private static void addAdditionalProperties(BoolQueryBuilder query, Entry<String, Object> entry,
        ...
        if (key.equalsIgnoreCase(JsonKey.FILTERS)) {
            ...
            for (Map.Entry<String, Object> en : filters.entrySet()) {
                ...
                || key.equalsIgnoreCase(JsonKey.NOT_EXISTS)) {
        ...
        if (val instanceof List) {
            if (!((List) val).isEmpty()) {
                if (((List) val).get(0) instanceof String) {
                    ...
                    query.must(createTermsQuery(key + RAW_APPEND, (List<String>) val,
            ...
            query.must(createTermQuery(key + RAW_APPEND, ((String) val).toLowerCase(),
                    constraintsMap.get(key)));
        } else {
    ...
    private static RangeQueryBuilder createRangeQuery(String name,
            Map<String, Object> rangeOperation, Float boost) {
            ...
            if (it.getKey().equalsIgnoreCase(STARTS_WITH)) {
                if (isNotNull(boost)) {
                    queryBuilder = QueryBuilders
                            .prefixQuery(key + RAW_APPEND, (String) it.getValue()).boost(boost);
                }
    ...
    public static boolean bulkInsertData(String index, String type,
        ...
        long startTime = System.currentTimeMillis();
        ProjectLogger.log("ElasticSearchUtil bulkInsertData method started at ==" + startTime
                + " for Type " + type, LoggerEnum.PERF_LOG);
        ...
        long stopTime = System.currentTimeMillis();
        long elapsedTime = stopTime - startTime;
        ProjectLogger.log("ElasticSearchUtil bulkInsertData method end at ==" + stopTime
                + " for Type " + type + " ,Total time elapsed = " + elapsedTime,
                LoggerEnum.PERF_LOG);
        return response;
    }

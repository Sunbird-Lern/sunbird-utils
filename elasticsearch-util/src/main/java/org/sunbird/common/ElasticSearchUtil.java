...
public class ElasticSearchUtil {
	...
	private static final List<String> upsertResults = new ArrayList<>(Arrays.asList("CREATED", "UPDATED", "NOOP"));
	...
	/**
	 * This method will put a new data entry inside Elastic search. identifier value
	 * becomes _id inside ES, so every time provide a unique value while saving it.
	 *
	 * @param index
	 ...
	 *            String ES type name
	 * @param identifier
	 *            ES column identifier as an String
	 * @param data
	 *            Map<String,Object>
	...
	public static String createData(String index, String type, String identifier, Map<String, Object> data) {
		...
		ProjectLogger.log("ElasticSearchUtil createData method started at ==" + startTime + " for Type " + type,
				LoggerEnum.PERF_LOG);
		...
		try {
			...
			ProjectLogger.log("ElasticSearchUtil createData method end at ==" + System.currentTimeMillis()
					+ " for Type " + type + " ,Total time elapsed = " + calculateEndTime(startTime),
					LoggerEnum.PERF_LOG);
	...
	public static Map<String, Object> getDataByIdentifier(String index, String type, String identifier) {
		long startTime = System.currentTimeMillis();
		ProjectLogger.log(
				"ElasticSearchUtil getDataByIdentifier method started at ==" + startTime + " for Type " + type,
				LoggerEnum.PERF_LOG);
	...
	public static Map<String, Object> searchData(String index, String type, Map<String, Object> searchData) {
		long startTime = System.currentTimeMillis();
		ProjectLogger.log("ElasticSearchUtil searchData method started at ==" + startTime + " for Type " + type,
				LoggerEnum.PERF_LOG);
		...
		long stopTime = System.currentTimeMillis();
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log("ElasticSearchUtil searchData method end at ==" + stopTime + " for Type " + type
				+ " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
		return sr.getAggregations().asList().get(0).getMetaData();
	}

	/**
	 * This method will update data based on identifier.take the data based on
	 * identifier and merge with incoming data then update it.
	 *
	 * @param index
	 *            String
	 * @param type
	...
	public static boolean updateData(String index, String type, String identifier, Map<String, Object> data) {
		long startTime = System.currentTimeMillis();
		ProjectLogger.log("ElasticSearchUtil updateData method started at ==" + startTime + " for Type " + type,
				LoggerEnum.PERF_LOG);
		if (!ProjectUtil.isStringNullOREmpty(index) && !ProjectUtil.isStringNullOREmpty(type)
			...
			verifyOrCreateIndexAndType(index, type);
			try {
				UpdateResponse response = ConnectionManager.getClient().prepareUpdate(index, type, identifier)
						.setDoc(data).get();
				ProjectLogger.log("updated response==" + response.getResult().name(), LoggerEnum.INFO.name());
				if (response.getResult().name().equals("UPDATED")) {
					long stopTime = System.currentTimeMillis();
					long elapsedTime = stopTime - startTime;
					ProjectLogger.log("ElasticSearchUtil updateData method end at ==" + stopTime + " for Type " + type
							+ " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
					return true;
				}
			} catch (Exception e) {
	...
	/**
	 * This method will upser data based on identifier.take the data based on
	 * identifier and merge with incoming data then update it and if identifier does
	 * not exist , it will insert data .
	 *
	 * @param index
	 *            String
	 * @param type
	 ...
	 *
	 * @param index
	 *            String name of the index which we need to delete.
	 * @return boolean
	 */
	...
	/**
	 * Method to perform the elastic search on the basis of SearchDTO . SearchDTO
	 * contains the search criteria like fields, facets, sort by , filters etc. here
	 * user can pass single type to search or multiple type or null
	 *
	 * @param type
	 *            var arg of String
	 * @return search result as Map.
	 */
	public static Map<String, Object> complexSearch(SearchDTO searchDTO, String index, String... type) {
		long startTime = System.currentTimeMillis();
		...
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log(
				"ElasticSearchUtil complexSearch method end at ==" + stopTime + " ,Total time elapsed = " + elapsedTime,
				LoggerEnum.PERF_LOG);
		return responsemap;
	}

	private static void addAggregations(SearchRequestBuilder searchRequestBuilder, List<Map<String, String>> facets) {
		long startTime = System.currentTimeMillis();
		...
		for (Map.Entry<String, String> entry : map.entrySet()) {
			...
			if (JsonKey.DATE_HISTOGRAM.equalsIgnoreCase(value)) {
				searchRequestBuilder.addAggregation(AggregationBuilders.dateHistogram(key).field(key + RAW_APPEND)
		...
		long stopTime = System.currentTimeMillis();
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log("ElasticSearchUtil addAggregations method end at ==" + stopTime + " ,Total time elapsed = "
	...
	/**
	 * Method to add the additional search query like range query , exists - not
	 * exist filter etc.
	 */
	@SuppressWarnings("unchecked")
	private static void addAdditionalProperties(BoolQueryBuilder query, Entry<String, Object> entry,
			Map<String, Float> constraintsMap) {
		long startTime = System.currentTimeMillis();
		ProjectLogger.log("ElasticSearchUtil addAdditionalProperties method started at ==" + startTime,
				LoggerEnum.PERF_LOG);
		String key = entry.getKey();
		...
		long stopTime = System.currentTimeMillis();
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log("ElasticSearchUtil addAdditionalProperties method end at ==" + stopTime
				+ " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
	}
	...
	private static void createFilterESOpperation(Entry<String, Object> entry, BoolQueryBuilder query,
		...
		if (val instanceof List) {
			...
			query.must(createTermQuery(key + RAW_APPEND, ((String) val).toLowerCase(), constraintsMap.get(key)));
		} else {
	...
	private static QueryBuilder createLexicalQuery(String key, Map<String, Object> rangeOperation, Float boost) {
		QueryBuilder queryBuilder = null;
		for (Map.Entry<String, Object> it : rangeOperation.entrySet()) {
			if (it.getKey().equalsIgnoreCase(STARTS_WITH)) {
				if (isNotNull(boost)) {
					queryBuilder = QueryBuilders.prefixQuery(key + RAW_APPEND, (String) it.getValue()).boost(boost);
				}
	 ...
	 * @return boolean
	 */
	public static boolean bulkInsertData(String index, String type, List<Map<String, Object>> dataList) {
		long startTime = System.currentTimeMillis();
		ProjectLogger.log("ElasticSearchUtil bulkInsertData method started at ==" + startTime + " for Type " + type,
				LoggerEnum.PERF_LOG);
		...
		try {
						...
						public void afterBulk(long executionId, BulkRequest request, BulkResponse response) {
							...
							if (bulkResponse != null) {
								while (bulkResponse.hasNext()) {
									...
									ProjectLogger.log("Bulk insert api response===" + bResponse.getId() + " "
											+ bResponse.isFailed());
								}
							}
						}
			...
			for (Map<String, Object> map : dataList) {
				...
				IndexRequest request = new IndexRequest(index, type, (String) map.get(JsonKey.IDENTIFIER)).source(map);
		...
		long stopTime = System.currentTimeMillis();
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log("ElasticSearchUtil bulkInsertData method end at ==" + stopTime + " for Type " + type
				+ " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
		return response;
	}
	...
	/**
	 * Method to execute ES query with the limitation of size set to 0 Currently
	 * this is a rest call
	 * 
	 * @param index
	 *            ES indexName
	...
	public static Response searchMetricsData(String index, String type, String rawQuery) {
		...
		try {
			...
			throw new ProjectCommonException(ResponseCode.unableToConnectToES.getErrorCode(),
					ResponseCode.unableToConnectToES.getErrorMessage(), ResponseCode.SERVER_ERROR.getResponseCode());
		} catch (Exception e) {
			throw new ProjectCommonException(ResponseCode.unableToParseData.getErrorCode(),
					ResponseCode.unableToParseData.getErrorMessage(), ResponseCode.SERVER_ERROR.getResponseCode());
		}
		Response response = new Response();
		response.put(JsonKey.RESPONSE, responseData);
		long stopTime = System.currentTimeMillis();
		long elapsedTime = stopTime - startTime;
		ProjectLogger.log("ElasticSearchUtil metrics search method end at == " + stopTime + " ,Total time elapsed = "
				+ elapsedTime, LoggerEnum.PERF_LOG);
		return response;
	}

	/**
	 * this method will take start time and subtract with current time to get the
	 * time spent in millis.
	 * 
	 * @param startTime
	 *            long
	 * @return long

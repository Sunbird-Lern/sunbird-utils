...
public class ElasticSearchUtil {
  ...
  public static String createData(String index, String type, String identifier,
    ...
    try {
      ...
      ProjectLogger.log(
          "ElasticSearchUtil createData method end at ==" + System.currentTimeMillis()
  ...
  public static Map<String, Object> getDataByIdentifier(String index, String type,
    ...
    ProjectLogger.log("ElasticSearchUtil getDataByIdentifier method end at ==" + stopTime
        + " for Type " + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
    ...
    if (!ProjectUtil.isStringNullOREmpty(index) && !ProjectUtil.isStringNullOREmpty(type)
      ...
      try {
        ...
        if (response.getResult().name().equals("UPDATED")) {
          long stopTime = System.currentTimeMillis();
          long elapsedTime = stopTime - startTime;
          ProjectLogger.log("ElasticSearchUtil updateData method end at ==" + stopTime
              + " for Type " + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
          return true;
        }
      } catch (Exception e) {
      ...
      if (createIndexResponse != null && createIndexResponse.isAcknowledged()) {
        ...
        if (!ProjectUtil.isStringNullOREmpty(mappings) && !ProjectUtil.isStringNullOREmpty(type)) {
          PutMappingResponse mappingResponse = client.admin().indices().preparePutMapping(index)
              .setType(type).setSource(mappings).get();
  ...
  public static Map<String, Object> complexSearch(SearchDTO searchDTO, String index,
    ...
    searchRequestBuilder.setFetchSource(
        searchDTO.getFields() != null ? searchDTO.getFields().stream().toArray(String[]::new)
            : null,
  ...
  private static void addAggregations(SearchRequestBuilder searchRequestBuilder,
    ...
    for (Map.Entry<String, String> entry : map.entrySet()) {
      ...
      if (JsonKey.DATE_HISTOGRAM.equalsIgnoreCase(value)) {
        searchRequestBuilder.addAggregation(AggregationBuilders.dateHistogram(key)
            .field(key + RAW_APPEND).dateHistogramInterval(DateHistogramInterval.days(1)));
  ...
  public static boolean bulkInsertData(String index, String type,
    ...
    try {
      BulkProcessor bulkProcessor =
          BulkProcessor.builder(ConnectionManager.getClient(), new BulkProcessor.Listener() {
            @Override
    ...
    long stopTime = System.currentTimeMillis();
    long elapsedTime = stopTime - startTime;
    ProjectLogger.log("ElasticSearchUtil bulkInsertData method end at ==" + stopTime + " for Type "
        + type + " ,Total time elapsed = " + elapsedTime, LoggerEnum.PERF_LOG);
    return response;
  }

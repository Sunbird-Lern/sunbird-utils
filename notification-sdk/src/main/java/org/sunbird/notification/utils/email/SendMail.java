...
public class SendMail {
	...
	public SendMail(EmailConfig config) {
		...
		this.password = 
		 StringUtils.isNotBlank(config.getPassword()) ? config.getPassword() :
		  Util.readValue(Constants.EMAIL_SERVER_PASSWORD);

		this.host = StringUtils.isNotBlank(config.getHost()) ? config.getHost()
				: Util.readValue(Constants.EMAIL_SERVER_HOST);
		this.port = StringUtils.isNotBlank(config.getPort()) ? config.getPort()
	...
	private boolean init() {
		...
		if (StringUtils.isBlank(host) || StringUtils.isBlank(port) || StringUtils.isBlank(userName)
				|| StringUtils.isBlank(password) || StringUtils.isBlank(fromEmail)) {
			logger.info("Email setting value is not provided by Env variable==" + host + " " + port + " " + fromEmail);
			response = false;
		} else {

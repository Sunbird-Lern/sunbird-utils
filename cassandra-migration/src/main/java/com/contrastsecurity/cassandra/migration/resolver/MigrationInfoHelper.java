...
/**
 * Parsing support for migrations that use the standard Flyway version + description embedding in
 * their name. These migrations have names like 1_2__Description .
 */
public class MigrationInfoHelper {
   ...
   * @throws CassandraMigrationException if the migration name does not follow the standard
   *     conventions.
   */
  public static Pair<MigrationVersion, String> extractVersionAndDescription(
    ...
    String description =
        cleanMigrationName.substring(descriptionPos + separator.length()).replaceAll("_", " ");

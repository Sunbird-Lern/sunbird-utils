...
public class FileSystemClassPathLocationScanner implements ClassPathLocationScanner {
   ...
   * @param scanRootLocation The root location of the scan on the classpath, without leading or
   *     trailing slashes.
  ...
  Set<String> findResourceNamesFromFileSystem(
      String classPathRootOnDisk, String scanRootLocation, File folder) throws IOException {
    LOG.debug(
      ...
      if (file.canRead()) {
        if (file.isDirectory()) {
          resourceNames.addAll(
              findResourceNamesFromFileSystem(classPathRootOnDisk, scanRootLocation, file));
        } else {

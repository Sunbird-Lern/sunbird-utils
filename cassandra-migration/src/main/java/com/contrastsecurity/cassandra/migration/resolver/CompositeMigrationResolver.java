...
/**
 * Facility for retrieving and sorting the available migrations from the classpath through the
 * various migration resolvers.
 */
public class CompositeMigrationResolver implements MigrationResolver {
  ...
  /**
   * The available migrations, sorted by version, newest first. An empty list is returned when no
   * migrations can be found.
   */
  ...
  private List<ResolvedMigration> doFindAvailableMigrations() throws CassandraMigrationException {
    List<ResolvedMigration> migrations =
        new ArrayList<ResolvedMigration>(collectMigrations(migrationResolvers));
   ...
   */
  /* private -> for testing */
  static Collection<ResolvedMigration> collectMigrations(
   ...
   * @throws CassandraMigrationException when two different migration with the same version number
   *     are found.
   */
  /* private -> for testing */

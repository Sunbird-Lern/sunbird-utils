...
public class CassandraMigration {
    ...
    <T> T execute(Action<T> action) {
        ...
        try {
            ...
            builder.addContactPoints(keyspace.getCluster().getContactpoints()).withPort(keyspace.getCluster().getPort()).withProtocolVersion(ProtocolVersion.V3).withRetryPolicy(DefaultRetryPolicy.INSTANCE);

...
public class CassandraStandaloneTest {
	...
	private static CassandraConnectionManagerImpl connectionManager = (CassandraConnectionManagerImpl) CassandraConnectionMngrFactory
			.getObject(cach.getProperty(JsonKey.SUNBIRD_CASSANDRA_MODE));
	...
	public void testConnection() {
		boolean bool = connectionManager.createConnection(host, port, "cassandra", "password", cassandraKeySpace);
		assertEquals(true, bool);
	}
	...
	public void testCgetPropertiesValueByIdOp() {
		Response response = operation.getPropertiesValueById(cassandraKeySpace, "address", "123", JsonKey.ID,
				JsonKey.CITY, JsonKey.ADD_TYPE);
		assertTrue(
				((String) ((List<Map<String, Object>>) response.get("response")).get(0).get(JsonKey.ID)).equals("123"));
	}
	...
	public void testDgetRecordByIdOp() {
		...
		assertTrue(((String) ((List<Map<String, Object>>) response.get("response")).get(0).get(JsonKey.CITY))
				.equals("city"));
	}

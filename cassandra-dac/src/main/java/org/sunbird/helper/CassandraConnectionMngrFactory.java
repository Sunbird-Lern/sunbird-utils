...
public class CassandraConnectionMngrFactory {
	...
	/**
	 * Factory method to get the cassandra connection manager oject on basis of mode
	 * name pass in argument . default mode is standalone mode .
	 * 
	 * @param name
	 * @return
	 */
			...
			synchronized (CassandraConnectionMngrFactory.class) {
				if (name.equalsIgnoreCase(JsonKey.EMBEDDED_MODE)) {
					...
					if (null == connectionFactoryMap.get(JsonKey.EMBEDDED_MODE)) {
						CassandraConnectionManager embeddedcassandraConnectionManager = new CassandraConnectionManagerImpl(
								mode);
					...
					if (null == connectionFactoryMap.get(JsonKey.STANDALONE_MODE)) {
						...
						CassandraConnectionManager standaloneCassandraConnectionManager = new CassandraConnectionManagerImpl(
								mode);

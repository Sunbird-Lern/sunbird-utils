...
public class CassandraConnectionManagerImpl implements CassandraConnectionManager {
	...
	/**
	 * Method to create the cassandra connection on the basis of mode i.e.
	 * standalone or embedde read from properties file .
	 * 
	 * @param ip
	 * @param port
	 * @param userName
	 ...
	 * @return
	 */
	private boolean createStandaloneConnection(String ip, String port, String userName, String password,
			String keyspace) {
	...
	/**
	 * Method to create the embedded cassandra connection on host{{read from
	 * external properties}} and port {{read from external properties}}.
	 * 
	 * @param ip
	 * @param port
	 * @param userName
		...
		if (null == cassandraSessionMap.get(keyspace)) {
			try {
				...
				Cluster cluster = new Cluster.Builder()
						...
						.withPort(Integer.parseInt(propertiesCache.getProperty(JsonKey.EMBEDDED_CASSANDRA_PORT)))
						.build();
				...
				dataLoader.load(
						new ClassPathCQLDataSet(propertiesCache.getProperty(JsonKey.EMBEDDED_CQL_FILE_NAME), keyspace));
				if (null != cassandraSession) {
					cassandraSessionMap.put(keyspace, cassandraSession);
					cassandraclusterMap.put(keyspace, cluster);
				}
	...
	public List<String> getTableList(String keyspacename) {
		Collection<TableMetadata> tables = cassandraclusterMap.get(keyspacename).getMetadata().getKeyspace(keyspacename)
				.getTables();

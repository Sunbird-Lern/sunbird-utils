 ...
 *
 * <p>The location type is determined by its prefix. Unprefixed locations or locations starting with
 * {@code classpath:} point to a package on the classpath and may contain both cql and java-based
 * migrations. Locations starting with {@code filesystem:} point to a directory on the filesystem
 * and may only contain cql migrations. (default: db/migration)
 */
public class ScriptsLocations {
   ...
   */
  private ScriptsLocation getParentLocationIfExists(
      ScriptsLocation location, List<ScriptsLocation> finalLocations) {

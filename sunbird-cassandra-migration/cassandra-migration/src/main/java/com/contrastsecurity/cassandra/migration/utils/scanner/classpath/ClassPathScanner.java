...
public class ClassPathScanner {
  ...
  /**
   * Scans the classpath for resources under the specified location, starting with the specified
  ...
  public Resource[] scanForResources(String path, String prefix, String suffix) throws IOException {
    LOG.debug(
        "Scanning for classpath resources at '"
            + path
            + "' (Prefix: '"
            + prefix
            + "', Suffix: '"
   ...
   *
   * @param location The location (package) in the classpath to start scanning. Subpackages are also
   *     scanned.
   ...
   */
  public Class<?>[] scanForClasses(String location, Class<?> implementedInterface)
      throws Exception {
    LOG.debug(
        "Scanning for classes at '"
            + location
            + "' (Implementing: '"
  ...
  private Set<String> findResourceNames(String path, String prefix, String suffix)
    ...
    for (URL locationUrl : locationsUrls) {
      ...
      if (classPathLocationScanner == null) {
        ...
        LOG.warn(
            "Unable to scan location: " + scanRoot + " (unsupported protocol: " + protocol + ")");
      } else {
    ...
    if (classLoader.getClass().getName().startsWith("com.ibm")) {
      ...
      if (!urls.hasMoreElements()) {
        throw new CassandraMigrationException(
            "Unable to determine URL for classpath location: "
                + path
                + " (ClassLoader: "
                + classLoader
                + ")"
      ...
      while (urls.hasMoreElements()) {
        ...
        locationUrls.add(
            new URL(
                URLDecoder.decode(url.toExternalForm(), "UTF-8").replace("/flyway.location", "")));
      }
    } else {

...
public class SchemaVersionDAO {
  ...
  public SchemaVersionDAO(Session session, Keyspace keyspace, String tableName) {
    ...
    this.consistencyLevel =
        session.getCluster().getMetadata().getAllHosts().size() > 1
  ...
  public boolean tablesExist() {
    ...
    Statement schemaVersionStatement =
        QueryBuilder.select().countAll().from(keyspace.getName(), tableName);

    Statement schemaVersionCountsStatement =
        QueryBuilder.select()
            .countAll()
    ...
    try {
      ...
      LOG.debug(
          "No schema version counts table found with a name of "
              + tableName
              + COUNTS_TABLE_NAME_SUFFIX);
    }
  ...
  public List<AppliedMigration> findAppliedMigrations() {
    ...
    // order by version_rank not necessary here as it eventually gets saved in TreeMap that uses
    // natural ordering
  ...
  private int calculateInstalledRank() {
    ...
    session.execute(statement);
    Select select = QueryBuilder.select("count").from(tableName + COUNTS_TABLE_NAME_SUFFIX);
  ...
  private int calculateVersionRank(MigrationVersion version) {
    ...
    for (Row versionRow : versionRows) {
      ...
      migrationMetaHolders.put(
          ...
          new MigrationMetaHolder(versionRow.getInt("version_rank")));
    }

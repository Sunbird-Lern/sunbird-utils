...
public class CqlStatementBuilder {
  ...
  /**
   * Analyses this line �nd extracts the new default delimiter. This method is only called between
   * statements and looks for explicit delimiter change directives.
   *
   ...
   * @return {@code true} if it is a directive that should be processed by the database, {@code
   *     false} if not.
   */
  ...
  public boolean canDiscard() {
    return !insideAlternateQuoteStringLiteral
        && !insideQuoteStringLiteral
  ...
  /**
   * Checks whether this line in the cql script indicates that the statement delimiter will be
   * different from the current one. Useful for database-specific stored procedures and block
   * constructs.
   *
   * @param line The simplified line to analyse.
   * @param delimiter The current delimiter.
   * @return The new delimiter to use (can be the same as the current one) or {@code null} for no
   *     delimiter.
   */
  ...
  protected void applyStateChanges(String line) {
    // Ignore all special characters that naturally occur in CQL, but are not opening or closing
    // string literals
   ...
   * @return The list of potentially delimiting string literals token types per token. Tokens that
   *     do not have any impact on string delimiting are discarded.
   */
      ...
      if (alternateQuote == null) {
        ...
        if (alternateQuoteFromToken != null) {
          ...
          if (cleanToken.length() >= (alternateQuoteFromToken.length() + closeQuote.length())
              && cleanToken.startsWith(alternateQuoteFromToken)
              && cleanToken.endsWith(closeQuote)) {
            // Skip $$abc$$, ...
            continue;
          }
  ...
  /**
   * Performs additional cleanup on this token, such as removing charset casting that prefixes
   * string literals. Must be implemented in dialect specific sub classes.
   *

...
public class Migrate {
  ...
  public int run() {
    ...
    while (true) {
      ...
      MigrationInfoService infoService =
          new MigrationInfoService(
              migrationResolver, schemaVersionDAO, target, allowOutOfOrder, true);
      infoService.refresh();
  ...
  private MigrationVersion applyMigration(final MigrationInfo migration, boolean isOutOfOrder) {
    ...
    try {
      ...
      LOG.debug(
          "Successfully completed and committed migration of keyspace "
              + schemaVersionDAO.getKeyspace().getName()
              + " to version "
              + version);
    } catch (CassandraMigrationException e) {
      String failedMsg =
          "Migration of keyspace "
              + schemaVersionDAO.getKeyspace().getName()
              + " to version "
              + version
              + " failed!";
  ...
  private void logSummary(int migrationSuccessCount, long executionTime) {
    ...
    if (migrationSuccessCount == 1) {
      LOG.info(
          "Successfully applied 1 migration to keyspace "
              + schemaVersionDAO.getKeyspace().getName()
              + " (execution time "

...
public class CassandraMigrationIT extends BaseIT {
  ...
  public void runApiTest() {
    ...
    for (MigrationInfo info : infoService.all()) {
      assertThat(
          info.getVersion().getVersion(), anyOf(is("1.0.0"), is("2.0.0"), is("3.0"), is("3.0.1")));
  ...
  public void runCmdTest() throws IOException, InterruptedException {
    String shell =
        "java -jar"
            ...
            + " -Dcassandra.migration.cluster.contactpoints="
            + BaseIT.CASSANDRA_CONTACT_POINT
            + " -Dcassandra.migration.cluster.port="
            + BaseIT.CASSANDRA_PORT
            + " -Dcassandra.migration.cluster.username="
            + BaseIT.CASSANDRA_USERNAME
            + " -Dcassandra.migration.cluster.password="
            + BaseIT.CASSANDRA_PASSWORD
            + " -Dcassandra.migration.keyspace.name="
            + BaseIT.CASSANDRA__KEYSPACE
    ...
    while (!runCmdTestCompleted) Thread.sleep(1000L);
